...
RegisterNetEvent('hospital:server:ambulanceAlert', function(text)
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
            TriggerClientEvent('hospital:client:ambulanceAlert', v.PlayerData.source, src, nil, coords, text)
        end
    end
end)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function()
    ...
    for k,v in pairs(players) do
        if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
			TriggerClientEvent('QBCore:Notify', v.PlayerData.source, 'Medical Staff is needed at Jackson Memorial!', 'ambulance')
		end
	end
end)
...
-- Commands
-- Added to tgiann-policealert for integration with PD calls

-- QBCore.Commands.Add('911e', 'EMS Report', {{name='message', help='Message to be sent'}}, false, function(source, args)
-- 	local src = source
-- 	if args[1] then message = table.concat(args, " ") else message = 'In need of assistance! Help!' end
--     local ped = GetPlayerPed(src)
--     local coords = GetEntityCoords(ped)
--     local players = QBCore.Functions.GetQBPlayers()
...
--         end
--     end
-- 	TriggerClientEvent('QBCore:Notify', src, "Emergency medical services have been notified", "success")
-- end)
...
QBCore.Functions.CreateUseableItem("firstaid", function(source, item)
	...
	if playerJob == ambulance or playerJob == police then
		...
		TriggerClientEvent('QBCore:Notify', src, "You don't have the right job for that!", "error")
	end
end) ]]
