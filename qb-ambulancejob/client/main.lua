...
WeaponDamageList = {
	...
	["WEAPON_BATTLEAXE"] = "Dent in your head from a stale baguette!",
    ...
    if not isDead and not InLaststand then
        if #injured > 0 then
            ...
            if #injured <= Config.AlertShowInfo then
                ...
                limbDamageMsg = "You feel pain on different limbs.."
            end
...
local function DoBleedAlert()
    if not isDead and tonumber(isBleeding) > 0 then
        QBCore.Functions.Notify(Config.BleedingStates[tonumber(isBleeding)].label, "error", 5000)
        TriggerEvent('hospital:client:bloodDroplets')
    end
end
...
RegisterNetEvent('hospital:client:ambulanceAlert', function(id, phone, coords, text)
    ...
    local street2 = GetStreetNameFromHashKey(var2)
    if street2 ~= nil and street2 ~= '' then street = 'Location: Between '..street..' and '..street2 else street = 'Location: On '..street end
    
    if phone then
        QBCore.Functions.Notify({text = 'Emergency: [ID: '..id..'] # '..phone..' | Call Info: '..text, caption = street}, 'ambulance')
    else
        QBCore.Functions.Notify({text = 'Distress Call [ID: '..id..']: '..text, caption = street}, 'ambulance')
    end
    ...
    local blipText = '* EMS Alert: [ID: '..id..'] Info:'..text
...
RegisterNetEvent('hospital:client:Revive', function()
    ...
    TriggerEvent('consumables:client:soberUp')
...
RegisterNetEvent('hospital:client:HealInjuries', function(type)
    ...
    TriggerServerEvent("hospital:server:RestoreWeaponDamage")
    QBCore.Functions.Notify("Your wounds have been treated!")
end)
...
RegisterNetEvent('hospital:client:SendToBed', function(id, data, isRevive)
    ...
    CreateThread(function ()
        ...
        if isRevive then
            if isDead then
                local healTime = (Config.AIHealTimer * 5000)
                QBCore.Functions.Notify("You are being treated, please wait for " .. math.floor(60 / Config.AIHealTimer * 5) .. " min(s)")
                QBCore.Functions.Progressbar("heal_time", "You are being treated", healTime, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() end)
                Citizen.Wait(Config.AIHealTimer * 5000)
            elseif InLaststand then
                local healTime = (Config.AIHealTimer * 4000)
                QBCore.Functions.Notify("You are being treated, please wait for  " .. math.floor(60 / Config.AIHealTimer * 4) .. " min(s)")
                QBCore.Functions.Progressbar("heal_time", "You are being treated", healTime, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                ...
                Citizen.Wait(Config.AIHealTimer * 4000)
            elseif isDamaged then
                local healTime = (Config.AIHealTimer * 2500)
                QBCore.Functions.Notify("You are being treated, please wait for " .. math.floor(60 / Config.AIHealTimer * 2.5) .. " min(s)")
                QBCore.Functions.Progressbar("heal_time", "You are being treated..", healTime, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
                ...
                Citizen.Wait( Config.AIHealTimer * 2500)
            else
                local healTime = (Config.AIHealTimer * 1000)
                QBCore.Functions.Notify("You are being treated, please wait for " .. math.floor(60 / Config.AIHealTimer * 1) .. " min(s)")
                QBCore.Functions.Progressbar("heal_time", "You are being treated..", healTime, false, true, {
                    disableMovement = true,
                    disableCarMovement = true,
                    disableMouse = false,
...
if deathTime > 0 then
                        ...
                        DrawTxt(0.865, 1.44, 1.0, 1.0, 0.6, "~w~ HOLD ~r~[E] ("..hold..")~w~ TO BE RUSHED TO THE HOSPITAL BY LOCALS ~r~($"..Config.BillCost..")~w~", 255, 255, 255, 255)
                    end
]]
...
RegisterNetEvent('hospital:client:SendBillEmail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = "Miami Medical Services",
            subject = "Hospital Bill",
            message = "Dear " .. gender .. " " .. charinfo.lastname .. ",<br /><br />Here is the receipt of your last visit to nearby Hospital.<br />The final total of your visit is: <strong>$"..amount.."</strong><br /><br />We wish you a quick and speedy recovery!",
            button = {}
        })
    end)
end)
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            local pos = GetEntityCoords(PlayerPedId())
            DrawText3D(pos.x, pos.y, pos.z, "Press ~g~E~w~ to get out of bed!")
            ...
            for k, checkins in pairs(Config.Locations["checking"]) do
                if #(pos - checkins) < 1.5 then
                    ...
                    if doctorCount >= Config.MinimalDoctors then
                        DrawText3D(checkins.x, checkins.y, checkins.z, "~g~E~w~ - Call Medical Staff")
                    else
                        DrawText3D(checkins.x, checkins.y, checkins.z, "~g~E~w~ - Get Admitted to the Emergency Room")
                    end
                    if IsControlJustReleased(0, 38) then
                        if doctorCount >= Config.MinimalDoctors then
