...
function SetLaststand(bool, spawn)
	...
	TriggerServerEvent('hospital:server:ambulanceAlert', 'Call for a person down at the location marked on your GPS. Get over there!')
    TriggerEvent('tgiann-policeAlert:alert', 'Reports of a <b><big>person</big></b> down coming in at the following location. Paramedics have also been alerted.', 'ems', 'PERSON IN DISTRESS', false, false, false)
            ...
            while InLaststand do
                ...
                elseif LaststandTime - 1 <= 0 then
                    if not isInHospitalBed then
                        ...
                        QBCore.Functions.Notify("You're still in a hospital bed, wait till the timer runs out... (max. 4 mins)", 10000)
                    end
                end
                Wait(1000)
            end
