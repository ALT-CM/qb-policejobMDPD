...
QBCore.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source)
    ...
    if Player then
        if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then --and Player.Functions.GetItemByName('spikestrip') then
...
QBCore.Commands.Add("grantlicense", "Grant a license to someone", {{name = "id", help = "ID of a person"}, {name = "license", help = "License Type"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 8 then
        if args[2] == "driver" or args[2] == "weapon" or args [2] == "sportsman" or args[2] == "commercial" or args[2] == "pilot" then
    ...
    elseif Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 1 then
            if args [2] == "sportsman" or args[2] == "commercial" then
                local SearchedPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
                if SearchedPlayer then
                    local licenseTable = SearchedPlayer.PlayerData.metadata["licences"]
                    licenseTable[args[2]] = true
    ...
    elseif PLayer.PlayerData.job.name == "adrenescape" and Player.PlayerData.job.grade.level >= 0 then
        if args[2] == "pilot" then
            local SearchedPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
            if SearchedPlayer then
                local licenseTable = SearchedPlayer.PlayerData.metadata["licences"]
                licenseTable[args[2]] = true
    ...
    -- elseif PLayer.PlayerData.job.name == "adrenalin" and Player.PlayerData.job.grade.level >= 0 then
    --     if args[2] == "driver" or args[2] == "weapon" or args [2] == "sportsman" or args[2] == "commercial" or args[2] == "pilot" then
    --         local SearchedPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    --         if SearchedPlayer then
    --             local licenseTable = SearchedPlayer.PlayerData.metadata["licences"]
...
QBCore.Commands.Add("shackle", "Cuff Player (Police Only)", {}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
-- Sam and Jah wrote this to test blood drops.
-- QBCore.Commands.Add("addblood", "Drop some blood", {}, true, function(source)
--     local src = source
--     local player = QBCore.Functions.GetPlayer(src)
--     local ped = GetPlayerPed(src)
--     local citizenid = player.PlayerData.citizenid
--     local bloodtype = player.PlayerData.metadata['bloodtype']
--     local bloodloc = GetEntityCoords(ped)
--     local coords = {
--         x = bloodloc.x,
--         y = bloodloc.y,
--         z = bloodloc.z,
--     }
--     TriggerEvent('evidence:server:CreateBloodDrop', citizenid, bloodtype, coords)
-- end)
...
QBCore.Commands.Add("takedna", "Take a DNA sample from a person (empty evidence bag needed) (Police Only)", {{name="id", help="ID of the person"}}, true, function(source, args)
    ...
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(args[1])))

    if Player.PlayerData.job.name == "police" then 
        if Player.PlayerData.job.grade.level >= 13 then 
            if Player.PlayerData.job.onduty then
                if dist < 2 then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "You are not close enough to this person", "error")
                end
            else
                TriggerClientEvent('QBCore:Notify', src, "You need to be on duty to do this!", "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "You are not a high enough grade to pull DNA..", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "You are not a cop..", "error")
    end
end)
...
QBCore.Commands.Add("911r", "Send a message back to a alert", {{name="id", help="ID of the alert"}, {name="message", help="Message you want to send"}}, true, function(source, args)
    ...
    local Prefix = "Police"
    local messagetype = "police"
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor" then
        if (Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.name == "doctor") then
            Prefix = "Ambulance"
            messagetype = "ambulance"
        end
        if OtherPlayer ~= nil then
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, "("..Prefix..") :" ..message, messagetype)
        ...
        TriggerClientEvent('QBCore:Notify', source, "You are not an Emergency Services employee...")
    end
end)
...
QBCore.Commands.Add("nearped", "Get the nearest person's ID", function(source)
    local src = source
    local players = {}
    for i = 0, 31 do
        if QBCore.Functions.NetworkIsPlayerActive(i) then
            table.insert(players, i)
        end
    end
    local closestDistance = -1
    ...
    TriggerClientEvent('QBCore:Notify', src, "Player's ID is (" .. closestPlayer .. ") testtest")
end)

    --print(closestPlayer)
    --TriggerClientEvent('QBCore:Notify', src, closestPlayer, "error")
--[[     local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local PlayerPeds = {}
    if next(PlayerPeds) == nil then
        for _, player in pairs(QBCore.Functions.GetPlayer()) do
            ...
            if closestPed then
                QBCore.Functions.Notify("closestPed")
                print(tostring(closestPed))
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "No one nearby")
    end ]]

	
    --local closest, distance = QBCore.Functions.GetClosestPlayer()
    --QBCore.Functions.Notify("ID number is:", closest)
    --TriggerClientEvent("qb-policejob:client:NearestPed")
   -- TriggerClientEvent("police:client:ClosestPed")


-- TAKEDNA original command
-- QBCore.Commands.Add("takedna", "Take a DNA sample from a person (empty evidence bag needed) (Police Only)", {{name="id", help="ID of the person"}}, true, function(source, args)
--     local src = source
--     local Player = QBCore.Functions.GetPlayer(src)
--     local OtherPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
...
QBCore.Commands.Add('duty', 'Toggle Duty (Admins Only)', {}, false, function(source)
	TriggerClientEvent('Toggle:Duty', source)
end, "admin")
...
QBCore.Functions.CreateUseableItem("shackles", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(item.name) then
        if Player.PlayerData.job.name == "police"  then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'For police only', 'error')
        end
    end
end)
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.Functions.GetItemByName("shackles") or Player.PlayerData.job.name == "police" then
            Wait(500)
...
RegisterNetEvent('evidence:server:StartBloodDrop', function(coords)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local ped = GetPlayerPed(src)
    local citizenid = player.PlayerData.citizenid
...
RegisterServerEvent('police:server:SendPoliceEmergencyAlert', function(streetLabel, coords, callsign)
    local data = {
        displayCode = "10-13",
