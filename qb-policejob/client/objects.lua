...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if PlayerJob.name == "police" and PlayerJob.onduty then
        ...
        QBCore.Functions.Progressbar("spikestrip", "Deploying Spike Strip..", 1500, false, true, {
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "police" and PlayerJob.onduty then
                            sleep = 0
                            DrawText3D(pos.x, pos.y, pos.z, '[E] Pickup Spikestrip')
                            if IsControlJustPressed(0, 38) then
                                ...
                                ClosestSpike = nil
                                TriggerServerEvent('QBCore:Server:AddItem', "spikestrip", 1)
                                TriggerServerEvent('police:server:SyncSpikes', SpawnedSpikes)
                            end
                        end
                    end
