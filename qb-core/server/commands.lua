...
QBCore.Commands.Add("givemoney", "Give A Player Money (Admin Only)", {{name="id", help="Player ID"},{name="moneytype", help="Type of money (cash, bank, crypto)"}, {name="amount", help="Amount of money"}}, true, function(source, args)
	...
	if Player ~= nil then
		...
		local amount = args[3]
		local type = args[2]
    	TriggerEvent('qb-log:server:CreateLog', 'staff', 'Give Money', 'lightgreen', string.format(
			'** %s %s ** gave **%s %s** __** $%s %s **__.\n\n```Steam: %s | SID: %s | CID: %s\n\nSteam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			targetPlayer.PlayerData.charinfo.firstname, targetPlayer.PlayerData.charinfo.lastname, 
			amount, type,
			GetPlayerName(senderPlayer.PlayerData.source), senderPlayer.PlayerData.source, senderPlayer.PlayerData.citizenid, 
			GetPlayerName(targetPlayer.PlayerData.source), targetPlayer.PlayerData.source, targetPlayer.PlayerData.citizenid), 
		false)
	else
		TriggerClientEvent('QBCore:Notify', source, "Player Not Online", "error")
	end
...
QBCore.Commands.Add("setmoney", "Set Players Money Amount (Admin Only)", {{name="id", help="Player ID"},{name="moneytype", help="Type of money (cash, bank, crypto)"}, {name="amount", help="Amount of money"}}, true, function(source, args)
	...
	if Player ~= nil then
		...
		local amount = args[3]
		local type = args[2]
    	TriggerEvent('qb-log:server:CreateLog', 'staff', 'Set Money', 'lightgreen', string.format(
			'** %s %s ** set **%s %s\'s** %s to __** $%s  **__.\n\n```Steam: %s | SID: %s | CID: %s\n\nSteam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			targetPlayer.PlayerData.charinfo.firstname, targetPlayer.PlayerData.charinfo.lastname, 
			type, amount,
			GetPlayerName(senderPlayer.PlayerData.source), senderPlayer.PlayerData.source, senderPlayer.PlayerData.citizenid, 
...
QBCore.Commands.Add("setjob", "Set A Players Job (Admin Only)", {{name="id", help="Player ID"}, {name="job", help="Job name"}, {name="grade", help="Grade"}}, true, function(source, args)
	...
	if Player == nil then
		...
		local job = args[2]
		local grade = args[3]
    	TriggerEvent('qb-log:server:CreateLog', 'staff', 'Set Job', 'yellow', string.format(
			'** %s %s ** set **%s %s\'s** job to ** %s ** grade **%s**.\n\n```Steam: %s | SID: %s | CID: %s\n\nSteam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			targetPlayer.PlayerData.charinfo.firstname, targetPlayer.PlayerData.charinfo.lastname, 
			job, grade,
			GetPlayerName(senderPlayer.PlayerData.source), senderPlayer.PlayerData.source, senderPlayer.PlayerData.citizenid, 
...
QBCore.Commands.Add("setgang", "Set A Players Gang (Admin Only)", {{name="id", help="Player ID"}, {name="gang", help="Name of a gang"}, {name="grade", help="Grade"}}, true, function(source, args)
	...
	if Player == nil then
		...
		local gang = args[2]
		local grade = args[3]
    	TriggerEvent('qb-log:server:CreateLog', 'staff', 'Set Gang', 'yellow', string.format(
			'** %s %s ** set **%s %s\'s** job to ** %s ** grade **%s**.\n\n```Steam: %s | SID: %s | CID: %s\n\nSteam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			targetPlayer.PlayerData.charinfo.firstname, targetPlayer.PlayerData.charinfo.lastname, 
			gang, grade,
...
QBCore.Commands.Add("clearinv", "Clear Players Inventory (Admin Only)", {{name="id", help="Player ID"}}, false, function(source, args)
	...
	if Player ~= nil then
		...
		local targetPlayer = Player
    	local senderPlayer = QBCore.Functions.GetPlayer(source)
    	TriggerEvent('qb-log:server:CreateLog', 'staff', 'Clear Inventory', 'red', string.format(
			'** %s %s ** cleared ** %s %s\'s ** inventory.\n\n```Steam: %s | SID: %s | CID: %s\n\nSteam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			targetPlayer.PlayerData.charinfo.firstname, targetPlayer.PlayerData.charinfo.lastname, 
			GetPlayerName(senderPlayer.PlayerData.source), senderPlayer.PlayerData.source, senderPlayer.PlayerData.citizenid, 
			GetPlayerName(targetPlayer.PlayerData.source), targetPlayer.PlayerData.source, targetPlayer.PlayerData.citizenid), 
...
QBCore.Commands.Add("ooc", "OOC Chat Message", {}, false, function(source, args)
	...
	for k, v in pairs(QBCore.Functions.GetPlayers()) do
		...
		local senderPlayer = QBCore.Functions.GetPlayer(source)
		TriggerEvent('qb-log:server:CreateLog', 'ooc', 'OOC', 'white', string.format(
			'** %s %s ** message: *%s*.\n\n```Steam: %s | SID: %s | CID: %s```',
			senderPlayer.PlayerData.charinfo.firstname, senderPlayer.PlayerData.charinfo.lastname, 
			message,
			GetPlayerName(senderPlayer.PlayerData.source), senderPlayer.PlayerData.source, senderPlayer.PlayerData.citizenid), 
