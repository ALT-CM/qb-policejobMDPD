...
function QBCore.Player.Login(source, citizenid, newData)
    ...
    if src then
        if citizenid then
			local result = exports.oxmysql:executeSync('SELECT * FROM players WHERE citizenid=@citizenid', {['@citizenid'] = citizenid})
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.charinfo.phone = PlayerData.charinfo.phone ~= nil and PlayerData.charinfo.phone or '786' .. math.random(1111111, 9999999)
    ...
    if PlayerData.metadata['jobrep'] ~= nil then
		PlayerData.metadata['jobrep'] = {
            ...
            ['delivery'] = PlayerData.metadata['jobrep']['delivery'] or 0,
...
function QBCore.Player.CreatePlayer(PlayerData)
    ...
    self.Functions.AddMoney = function(moneytype, amount, reason)
        ...
        if self.PlayerData.money[moneytype] then
            ...
            self.Functions.UpdatePlayerData()
            if amount > 90000 then -- tag everyone, default = 100000
                --TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Money Added', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype], true)

                --local Player = QBCore.Functions.GetPlayer(src)

                -- Nala Roi received $300 cash. New cash balance: $500000.
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Money Added', 'white', string.format('** %s %s ** received __** $%s in %s **__. New %s balance: __** $%s **__.\n\n```Steam: %s | SID: %s | CID: %s```', 
                self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                amount, moneytype, moneytype, self.PlayerData.money[moneytype],
                GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), true)
            else
                ...
                self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                amount, moneytype, moneytype, self.PlayerData.money[moneytype],
                GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), false)
            end
    ...
    self.Functions.RemoveMoney = function(moneytype, amount, reason)
        ...
        if self.PlayerData.money[moneytype] then
            ...
            self.Functions.UpdatePlayerData()
            if amount > 90000 then -- tag everyone, default = 100000
                --TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'white', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype], true)

                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Money Removed', 'white', string.format('** %s %s ** removed __** $%s in %s **__. New %s balance: __** $%s **__.\n\n```Steam: %s | SID: %s | CID: %s```', 
                self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                amount, moneytype, moneytype, self.PlayerData.money[moneytype],
                GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), true)
            else
                --TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'white', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype])
    ...
    self.Functions.SetMoney = function(moneytype, amount, reason)
        ...
        if self.PlayerData.money[moneytype] then
            ...
            TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Money Set', 'white', string.format('** %s %s ** set __** $%s in %s **__. New %s balance: __** $%s **__.\n\n```Steam: %s | SID: %s | CID: %s```', 
                self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                amount, moneytype, moneytype, self.PlayerData.money[moneytype], reason,
                GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), false)
            return true
        end
        return false
    ...
    self.Functions.AddItem = function(item, amount, slot, info)
        ...
        if (totalWeight + (itemInfo['weight'] * amount)) <= QBCore.Config.Player.MaxWeight then
            if (slot and self.PlayerData.items[slot]) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Item Added', 'lightgreen', string.format(
                    '** %s %s ** got **%sX %s**. New total amount: *%s*.\n\n```Steam: %s | SID: %s | CID: %s```', 
                    self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                    amount, self.PlayerData.items[slot].label, self.PlayerData.items[slot].amount,
                    GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), false)
            ...
            elseif (itemInfo['unique']) or (not slot or slot == nil) or (itemInfo['type'] == 'weapon') then
                for i = 1, QBConfig.Player.MaxInvSlots, 1 do
                    if self.PlayerData.items[i] == nil then
                        ...
                        --TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** got item: [slot:' .. i .. '], itemname: ' .. self.PlayerData.items[i].label .. ', added amount: ' .. amount .. ', new total amount: ' .. self.PlayerData.items[i].amount)
                        
                        TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Item Added', 'lightgreen', string.format(
                            '** %s %s ** got **%sX %s**. New total amount: *%s*.\n\n```Steam: %s | SID: %s | CID: %s```', 
                            self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                            amount, self.PlayerData.items[i].label, self.PlayerData.items[i].amount,
                            GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), false)
    ...
    self.Functions.RemoveItem = function(item, amount, slot)
        ...
        if slot then
            if self.PlayerData.items[slot].amount > amount then
                self.PlayerData.items[slot].amount = self.PlayerData.items[slot].amount - amount
                self.Functions.UpdatePlayerData()
                --('qb-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. self.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. self.PlayerData.items[slot].amount)

                TriggerEvent('qb-log:server:CreateLog', 'playerinventory', 'Item Removed', 'orange', string.format(
                            '** %s %s ** lost **%sX %s**. New total amount: *%s*.\n\n```Steam: %s | SID: %s | CID: %s```', 
                            self.PlayerData.charinfo.firstname, self.PlayerData.charinfo.lastname, 
                            amount, self.PlayerData.items[slot].label, self.PlayerData.items[slot].amount,
                            GetPlayerName(self.PlayerData.source), self.PlayerData.source, self.PlayerData.citizenid), false)
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
		...
		if result[1] == nil then
			exports.oxmysql:execute('INSERT INTO players (citizenid, cid, license, name, money, charinfo, job, gang, position, metadata) VALUES (@citizenid, @cid, @license, @name, @money, @charinfo, @job, @gang, @position, @metadata)', {
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        local Player = QBCore.Functions.GetPlayer(src)
            TriggerEvent('qb-log:server:CreateLog', 'staff', 'Character Deleted', 'red', string.format('** %s %s ** was deleted from %s\'s account.\n\n```Steam: %s | SID: %s | CID: %s```', 
            Player.PlayerData.charinfo.firstname, Player.PlayerData.charinfo.lastname, GetPlayerName(Player.PlayerData.source), GetPlayerName(Player.PlayerData.source), Player.PlayerData.source, Player.PlayerData.citizenid), false)
